...
터미널 환경에서 대화형(CLI)으로 동작하며, 메인 메뉴의 번호 선택을 통해 기능을 실행함 (`prompts.json` 파일과 연동되어 데이터 영구 보존).

### 🎬 실행 및 메뉴별 기능
1. **프로그램 구동**
   * 터미널 명령어 입력: `python3 prompt_manager.py`
   * 실행 시 0번부터 6번까지의 메인 메뉴 출력

2. **[1] 프롬프트 추가 (Add)**
   * 입력: `1` 입력 ➔ 제목, 내용, 카테고리 작성
   * 결과: 새로운 프롬프트가 추가되며 `prompts.json`에 실시간 저장됨

3. **[2] 프롬프트 목록 조회 (List)**
   * 입력: `2` 입력
   * 결과: 전체 프롬프트의 번호, 즐겨찾기(⭐), 제목, 카테고리 목록 출력

4. **[3] 프롬프트 검색 (Search)**
   * 입력: `3` 입력 ➔ 검색 키워드 입력
   * 결과: 제목이나 내용에 키워드가 포함된 항목을 실시간 필터링하여 출력

5. **[4] 상세 보기 (Detail)**
   * 입력: `4` 입력 ➔ 확인할 프롬프트 번호 입력
   * 결과: 선택한 항목의 전체 내용 및 상세 정보 출력

6. **[5] 즐겨찾기 토글 (Toggle Favorite)**
   * 입력: `5` 입력 ➔ 번호 입력
   * 결과: 해당 항목의 즐겨찾기 상태(True/False) 반전 및 파일 즉시 반영

7. **[6] 카테고리별 조회 (Category Filter)**
   * 입력: `6` 입력 ➔ 카테고리 명칭 입력 (예: `텍스트 생성`)
   * 결과: 해당 카테고리에 속한 프롬프트들만 모아서 출력
...
## 4. 파이썬 코드 상세 설명 (`prompt_manager.py`)
